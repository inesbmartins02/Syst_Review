...
def read_megapath_output(file_path):
    megapath_data = []
    unique_gcfs = set()
    with open(file_path, 'r') as f:
        ...
        for row in reader:
            megapath_data.append(row)
            unique_gcfs.add(row['assembly_id'])
    return megapath_data, unique_gcfs
...
def calculate_metrics(megapath_data, taxonomy_mapping, unique_gcfs):
    metrics = defaultdict(lambda: {'correct': 0, 'incorrect': 0, 'not_classified': 0, 'total': 0})
    correctly_classified_gcfs = set()
    unclassified_gcfs = set(taxonomy_mapping.keys()) - unique_gcfs

    gcf_classifications = defaultdict(lambda: defaultdict(list))

    for entry in megapath_data:
        gcf_id = entry['assembly_id']
    ...
    for gcf_id in unclassified_gcfs:
        for level in ['Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species']:
            metrics[level]['total'] += 1
            metrics[level]['incorrect'] += 1
            metrics[level]['not_classified'] += 1
    ...
    print(f"Número de GCFs não classificados: {len(unclassified_gcfs)}")
...
def main():
    megapath_file_path = input("Enter the path to the MegaPath-Nano output transformed file (e.g., output_mpn.csv): ")
    taxonomy_file_path = input("Enter the path to the taxonomy mapping file (e.g., archaea_taxonomy_mapping.csv): ")

    megapath_data, unique_gcfs = read_megapath_output(megapath_file_path)
    taxonomy_mapping = read_taxonomy_mapping(taxonomy_file_path)

    metrics = calculate_metrics(megapath_data, taxonomy_mapping, unique_gcfs)
